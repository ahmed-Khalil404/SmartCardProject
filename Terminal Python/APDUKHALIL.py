...
def makePayment(self):
    response, resp_word1, resp_word2 = self.pullData(0xC1, self.logging_enabled)
...
def chargeCard(self):
    ...
    def startPINSession(self, pin_code):
        _, resp_word1, resp_word2 = self.pushData(0XA0, pin_code, self.logging_enabled)
        if self.verifyResponse(resp_word1, resp_word2) == 0:
    ...
    def terminatePINSession(self):
        _, resp_word1, resp_word2 = self.pushEmptyCommand(0XA1, self.logging_enabled)
        if self.verifyResponse(resp_word1, resp_word2) == -1:
            print("PIN session terminated successfully")
        else:
    ...
    def modifyPIN(self, new_pin):
        if len(new_pin) != 6:
            ...
            _, resp_word1, resp_word2 = self.pushData(0XA2, new_pin, self.logging_enabled)
            if self.verifyResponse(resp_word1, resp_word2) == -1:
    ...
    def generateRSAKeyPair(self):
        _, resp_word1, resp_word2 = self.pushEmptyCommand(0xB0, self.logging_enabled)
        if self.verifyResponse(resp_word1, resp_word2) == 0:
    ...
    def retrieveRSAPublicKey(self):
        key_data, resp_word1, resp_word2 = self.pullData(0xB1, 0x47, self.logging_enabled)
        if self.verifyResponse(resp_word1, resp_word2) == 0:
    ...
    def signMessageWithRSA(self, message):
        _, resp_word1, resp_word2 = self.pushData(0xB2, message, self.logging_enabled)
        if self.verifyResponse(resp_word1, resp_word2) == -1:
            print("PIN session not active")
        elif self.verifyResponse(resp_word1, resp_word2) == -3:
            print("Message must be under 127 characters")
        else:
            signature_data, resp_word1_2, resp_word2_2 = self.pullData(0xB3, resp_word2, self.logging_enabled)
